...
engine = PPStructureV3(use_chart_recognition=True, lang='vi')
print("Model loaded successfully.")

parent_path = os.path.dirname(os.path.abspath(__file__))
# img_path = os.path.join(os.path.dirname(os.path.abspath(parent_path)), "assets", "image.png")
img_path = os.path.join(os.path.dirname(os.path.abspath(parent_path)), "assets", "test", "3.png")

def process(region: dict, img):
    output_dir = os.path.join(os.path.dirname(os.path.abspath(parent_path)), "results")
    os.makedirs(output_dir, exist_ok=True)
    # --- 1. HANDLE TEXT (from parsing_res_list) ---
    print("--- Text Extraction ---")
    structure_list = region.get('parsing_res_list', [])
    result = region['layout_det_res']['boxes']
    ...
    for i, img_item in enumerate(images_list):
        ...
        if pil_image:
            save_path = os.path.join(output_dir, "images", f"extracted_figure_{i}.jpg")
    ...
    folder_path = os.path.join(output_dir, "tables")
    os.makedirs(folder_path, exist_ok=True)
    for i, table in enumerate(table_list):
        ele = table['pred_html']
        with open(os.path.join(output_dir, "tables", f"extracted_table_{i}.html"), "w") as f:
            f.write(ele)
    ...
    folder_path = os.path.join(output_dir, "charts")
    os.makedirs(folder_path, exist_ok=True)
    ...
    for i in result:
        ...
        x1, y1, x2, y2 = [int(c) for c in i['coordinate']]
        # --- OPTIONAL: Add Padding/Margin ---
        # Charts often have labels on the edge. Let's add 10 pixels of white space.
        h, w, _ = img.shape
        padding = 10
        x1 = max(0, x1 - padding)
        y1 = max(0, y1 - padding)
        ...
        # Crop the image: img[y:y+h, x:x+w]
        chart_image = img[y1:y2, x1:x2]

        # Save file
        filename = os.path.join(folder_path, f"extracted_chart_{count}.jpg")
    ...
    folder_path = os.path.join(output_dir, "formulas")
    os.makedirs(folder_path, exist_ok=True)
    count = len(os.listdir(folder_path))
    for i in result:
        if i['label'] != 'formula':
        ...
        # Crop the image: img[y:y+h, x:x+w]
        form_image = img[y1:y2, x1:x2]

        # Save file
        filename = os.path.join(folder_path, f"extracted_formula_{count}.jpg")
        cv2.imwrite(filename, form_image)
        print(f"Successfully saved: {filename}")
        count += 1
...
def extract_non_text_elements(img_path):
    ...
    print(f"Found {len(result)} region{'s' if len(result) != 1 else ''}.")
