    ...
    try:
        ...
        if max_size >= 1920:
            ...
            img_original = cv2.resize(
                img_original, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC
            )
        ...
        for region_idx, region in enumerate(result):
            ...
            for i, img_item in enumerate(images_list):
                ...
                if pil_image:
                    save_path = os.path.join(
                        extracted_images_dir, f"region_{region_idx}_img_{i}.jpg"
                    )
            ...
            for i, box in enumerate(det_boxes):
                ...
                if label in ["figure", "table", "equation", "chart", "formula"]:
                    ...
                    save_path = os.path.join(
                        extracted_images_dir, f"region_{region_idx}_{label}_{i}.jpg"
                    )
